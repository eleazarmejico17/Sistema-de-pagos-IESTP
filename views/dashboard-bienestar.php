...
switch($pagina){
    ...
    case 'reportes-bienestar-estudiantil':
        ...
        $icono = 'fa-chart-bar';
        break;
    case 'solicitud-bienestar-estudiantil':
        $titulo = 'SOLICITUDES';
        $icono = 'fa-file-alt';
        break;
    default:
...
<div class="flex">
  ...
  <aside id="sidebar" class="fixed left-0 top-0 h-full w-72 bg-gradient-to-b from-[#0f172a] via-[#1e293b] to-black text-white shadow-2xl z-50 rounded-tr-[70px] rounded-br-[70px] overflow-hidden backdrop-blur-lg transition-transform duration-700 ease-out translate-x-0">
    ...
    <nav class="flex flex-col gap-3 px-5">
      <button onclick="window.location='?pagina=registro-bienestar-estudiantil'" class="relative flex items-center gap-4 px-5 py-3 rounded-2xl font-semibold transition-all duration-500 hover:translate-x-2 overflow-hidden group <?= activo('registro-bienestar-estudiantil', $pagina) ?>">
      ...
      <button onclick="window.location='?pagina=reportes-bienestar-estudiantil'" class="relative flex items-center gap-4 px-5 py-3 rounded-2xl font-semibold transition-all duration-500 hover:translate-x-2 overflow-hidden group <?= activo('reportes-bienestar-estudiantil', $pagina) ?>">
        <span class="absolute inset-0 bg-gradient-to-r from-blue-600 to-blue-400 opacity-0 group-hover:opacity-100 blur-xl transition-all duration-700"></span>
        <span class="absolute inset-0 bg-white/5 group-hover:bg-white/10 rounded-2xl transition-all duration-700"></span>
      ...
      <button onclick="window.location='?pagina=solicitud-bienestar-estudiantil'" class="relative flex items-center gap-4 px-5 py-3 rounded-2xl font-semibold transition-all duration-500 hover:translate-x-2 overflow-hidden group <?= activo('solicitud-bienestar-estudiantil', $pagina) ?>">
        <span class="absolute inset-0 bg-gradient-to-r from-blue-600 to-blue-400 opacity-0 group-hover:opacity-100 blur-xl transition-all duration-700"></span>
        <span class="absolute inset-0 bg-white/5 group-hover:bg-white/10 rounded-2xl transition-all duration-700"></span>
        <i class="fas fa-file-alt text-xl relative z-10"></i>
        <span class="relative z-10">SOLICITUDES</span>
      </button>
    </nav>
